...
@SuppressLint("NewApi")
public class DragDropGrid extends ViewGroup implements View.OnLongClickListener, OnTouchListener,
    ...
    public void setEditMode(boolean isEditMode){
    	...
    	for(int i=0;i<getChildCount();i++){
    		((DragView)getChildAt(i)).setEditMode(mIsEditMode);    		
    	}
    }
    
    public void setViewChange(){
    	for(int i=0;i<getChildCount();i++){
    		((DragView)getChildAt(i)).setViewChange();    		
    	}
					 ...
					 final int action = event.getAction();
				        ...
				        case DragEvent.ACTION_DRAG_LOCATION:
				            if (mDraggingView != null) {
                                ...
                                final int SCROLL_MOVE_POINT = ((ScrollView)getParent()).getHeight() / 5;

				                if(event.getY() < SCROLL_MOVE_POINT)
				                	((ScrollView)getParent()).fullScroll(ScrollView.FOCUS_UP);
				                else if(event.getY() > getMeasuredHeight() - SCROLL_MOVE_POINT)
				                	((ScrollView)getParent()).fullScroll(ScrollView.FOCUS_DOWN);
				                else if(event.getY() - ((ScrollView)getParent()).getScrollY() < SCROLL_MOVE_POINT)
				                	((ScrollView)getParent()).scrollBy(0, -20);
				                else if(event.getY() - ((ScrollView)getParent()).getScrollY() > ((ScrollView)getParent()).getHeight() - SCROLL_MOVE_POINT)
				                	((ScrollView)getParent()).scrollBy(0, 20);
				                
				                
//				                if( event.getY() + (mHeight- mGetLawY) + 300 > mHeight ){
//				                	
//				                	
//				                	((ScrollView)getParent()).scrollTo(0,((ScrollView)getParent()).getScrollY() +10); 
//				                }else if(event.getY() -mGetLawY  < 300){
//				                	((ScrollView)getParent()).scrollTo(0,((ScrollView)getParent()).getScrollY() -10); 
//				                }
				                
				                /*else if(mGetLawY < 400){
				                	if(mMoveLenght!=0){
				                		mMoveLenght =  mMoveLenght - 10;
				                		((ScrollView)getParent()).smoothScrollTo(0,((ScrollView)getParent()).getScrollY() -10);
				                	}
				              ...
				              /*  if (height - y < THRESHHOLD) {
				                	((ScrollView)getParent()).smoothScrollTo(0,((ScrollView)getParent()).getScrollY() +30);                    
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            if(index !=-1){
            	...
            	((ImageView)view.findViewById(R.id.cardImg)).setSelected(true);
        ...
        case MotionEvent.ACTION_CANCEL:
        	...
        	for(int i=0;i<getChildCount();i++){
            	...
            	((ImageView)view.findViewById(R.id.cardImg)).setSelected(false);
    ...
    public int positionForView(View v) {
    ...
    public int indexForView(View v) {
        for (int index = 0; index < getItemViewCount(); index++) {
            View child = getChildView(index);
            ...
            if (isPointInsideView(location[0]+10, location[1]+10, child)) {
                return index;
            }
        }
        return -1;
    ...
    public boolean onLongClick(View v) {
        ...
        if(mIsEditMode){
        	...
        	if(mDragSource!=null){
        		mDragSource.setonItemLongClickListener(index);
        	}
        }
    ...
    public void onClick(View v) {
        ...
        if(mIsEditMode){
	        ...
	        if (mDragSource != null && view instanceof DragAndDropElement) {
	            ...
	            mDragSource.setOnItemClickListener(index);
	        }
        }else{
        	if(mDragSource!=null){
        		mDragSource.setOnItemClickListener(index);
    ...
    private void transferChildView(int baseIndex) {
        ...
        FavoriteAndHistoryItem orifromItem = ((DragView) viewWithIndex(fromIndex)).getItem();
        ...
        if (toIndex > fromIndex) {
            for (int i = fromIndex + 1; i < toIndex + 1; i++) {
                ...
                DragView toDragView = (DragView) getChildView(moveIndex);
                DragView fromDragView = (DragView) viewWithIndex(i);
                
                FavoriteAndHistoryItem toItem = toDragView.getItem();
                FavoriteAndHistoryItem fromItem = fromDragView.getItem();
                        
                
                
                ((DragView) tb).setItem(fromItem);                
        ...
        ((DragView) dragTabView).setItem(orifromItem); 
