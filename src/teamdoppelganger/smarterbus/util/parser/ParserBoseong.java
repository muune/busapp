...
public class ParserBoseong extends CommonParser {


    public ParserBoseong(SQLiteDatabase db) {
        super(db);
    }
    ...
    public DepthStopItem getStopList(BusRouteItem rItem) {
        ...
        String selectBusStopQry = String.format("SELECT %s,%s FROM %s where %s='%s'", CommonConstants.BUS_ROUTE_RELATED_STOPS, CommonConstants.BUS_ROUTE_TURN_STOP_IDX, CommonConstants.CITY_BO_SEONG._engName + "_Route",
        ...
        if (cursor.moveToNext()) {
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                String stopQry = String.format("SELECT * From %s where %s=%s", CommonConstants.CITY_BO_SEONG._engName + "_Stop",
                ...
                if (stopCursor.moveToNext()) {
                    ...
                    stopItem.localInfoId = String.valueOf(CommonConstants.CITY_BO_SEONG._cityId);
    ...
    public DepthRouteItem getLineList(BusStopItem sItem) {
        ...
        String url = "http://bus.boseong.go.kr:8282/inq/searchBusStopRoute.do";
        ...
        if (parseStr != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    routeItem.localInfoId = String.valueOf(CommonConstants.CITY_BO_SEONG._cityId);


                    if (min.contains("약")) {
                        ArriveItem arriveItem = new ArriveItem();
                        arriveItem.remainMin = Integer.parseInt(min.split("약")[1].split("분후")[0]);
                        arriveItem.remainStop = Integer.parseInt(where.split("전")[0]);
    ...
    public DepthStopItem getDepthStopList(DepthStopItem sItem) {
        ...
        String htmlResult = RequestCommonFuction.getSource("http://bus.boseong.go.kr:8282/inq/searchBusRealLocationDetail.do", true, "busRouteId=" + sItem.tempId, "utf-8");
        ...
        if (htmlResult != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    carNum = carNum.substring(0, 3) + " " + carNum.substring(3, carNum.length());
