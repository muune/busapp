...
public class ParserYeosu extends CommonParser {


    public ParserYeosu(SQLiteDatabase db) {
        super(db);
    ...
    public DepthStopItem getStopList(BusRouteItem rItem) {
        ...
        String selectBusStopQry = String.format("SELECT %s,%s FROM %s where %s='%s'", CommonConstants.BUS_ROUTE_RELATED_STOPS, CommonConstants.BUS_ROUTE_TURN_STOP_IDX, CommonConstants.CITY_YEO_SU._engName + "_Route",
        ...
        if (cursor.moveToNext()) {
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                String stopQry = String.format("SELECT * From %s where %s=%s", CommonConstants.CITY_YEO_SU._engName + "_Stop",
                ...
                if (stopCursor.moveToNext()) {
                    ...
                    stopItem.localInfoId = String.valueOf(CommonConstants.CITY_YEO_SU._cityId);
    ...
    public DepthStopItem getDepthStopList(DepthStopItem sItem) {
        ...
        String htmlResult = RequestCommonFuction.getSource("http://its.yeosu.go.kr:9090/bis-yeosu/inq/searchBusRealLocationDetail.do", true, "busRouteId=" + sItem.tempId, "utf-8");
    ...
    public DepthRouteItem getLineList(BusStopItem sItem) {
        ...
        String url = "http://its.yeosu.go.kr:9090/bis-yeosu/inq/searchBusStopRoute.do";
        ...
        if (parseStr != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    if (where.contains("전")) {
                        where = where.split("전")[0];
                    }

                    routeItem.busRouteName = name;
                    routeItem.busRouteApiId = object.getString("route_id");
                    routeItem.localInfoId = String.valueOf(CommonConstants.CITY_YEO_SU._cityId);


                    if (min.contains("잠시후도착")) {
