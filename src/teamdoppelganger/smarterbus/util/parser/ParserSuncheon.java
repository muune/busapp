...
public class ParserSuncheon extends CommonParser {

    public ParserSuncheon(SQLiteDatabase db) {
        super(db);
    }
    ...
    public DepthStopItem getStopList(BusRouteItem rItem) {
        ...
        String selectBusStopQry = String.format("SELECT %s,%s FROM %s where %s='%s'", CommonConstants.BUS_ROUTE_RELATED_STOPS, CommonConstants.BUS_ROUTE_TURN_STOP_IDX, CommonConstants.CITY_SUN_CHEON._engName + "_Route",
        ...
        if (cursor.moveToNext()) {
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                String stopQry = String.format("SELECT * From %s where %s=%s", CommonConstants.CITY_SUN_CHEON._engName + "_Stop",
                ...
                if (stopCursor.moveToNext()) {
                    ...
                    stopItem.localInfoId = String.valueOf(CommonConstants.CITY_SUN_CHEON._cityId);
    ...
    public DepthStopItem getDepthStopList(DepthStopItem sItem) {
        ...
        String htmlResult = RequestCommonFuction.getSource("http://its.sc.go.kr/inq/searchBusRealLocationDetail.do", true, "busRouteId=" + sItem.tempId, "utf-8");
    ...
    public DepthRouteItem getLineList(BusStopItem sItem) {
        ...
        String url = "http://its.sc.go.kr/inq/searchBusStopRoute.do";
        ...
        if (parseStr != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    routeItem.localInfoId = String.valueOf(CommonConstants.CITY_SUN_CHEON._cityId);


                    if (min.contains("약")) {
                        ArriveItem arriveItem = new ArriveItem();
                        arriveItem.remainMin = Integer.parseInt(min.split("약")[1].split("분후")[0]);
                        arriveItem.remainStop = Integer.parseInt(where.split("정류소전")[0]);
        ...
        if (parseStr != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    if (min.contains("약") && name.equals(sItem.busRouteName)) {
                        ArriveItem arriveItem = new ArriveItem();
                        arriveItem.remainStop = Integer.parseInt(where.split("정류소전")[0]);
                        arriveItem.remainMin = Integer.parseInt(min.split("약")[1].split("분후")[0]);
                        routeItem.arriveInfo.add(arriveItem);
