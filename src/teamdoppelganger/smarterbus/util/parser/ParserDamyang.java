...
public class ParserDamyang extends CommonParser {


    public ParserDamyang(SQLiteDatabase db) {
        super(db);
    }
    ...
    public DepthStopItem getStopList(BusRouteItem rItem) {
        ...
        String url = String
                .format("http://bis.damyang.go.kr/app/mobile/bus_realtime/bus_location2.jsp?lID=%s&hdMode=NAME_L&txtKeyword=%s",
        ...
        if (htmlResult != null) {
            ...
            for (int j = 0; j < ddTag.size(); j++) {
                ...
                stopItem.localInfoId = String.valueOf(CommonConstants.CITY_DAMYANG._cityId);
    ...
    public DepthRouteItem getLineList(BusStopItem sItem) {
        ...
        String parseStr = RequestCommonFuction.getSource("http://bis.damyang.go.kr/gjmap/realTimeBusInfo.do?action=stationArriveInfo", true, param, "utf-8");
        ...
        if (parseStr != null) {
            ...
            for (int i = 0; i < elements.size(); i++) {
                try {
                    ...
                    String num = elements.get(i).text().split("번")[0].trim();
                    String where = "";
                    String min = "";

                    ArriveItem arriveItem = new ArriveItem();

                    if (elements.get(i).text().contains("잠시 후")) {
                        where = "0";
                        min = "0";
                        arriveItem.state = Constants.STATE_NEAR;
                    } else {
                        where = elements.get(i).text().substring(lastindex1, elements.get(i).text().length() - 1).split("\\(")[1].split(" ")[0];
                        min = elements.get(i).text().substring(lastindex2, elements.get(i).text().length() - 1).split("\\)")[1].split("분")[0].split(" ")[2];
                    ...
                    routeItem.localInfoId = String.valueOf(CommonConstants.CITY_DAMYANG._cityId);
